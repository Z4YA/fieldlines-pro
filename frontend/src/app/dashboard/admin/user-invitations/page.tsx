...
export default function UserInvitationsPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    const response = await api.createUserInvitation(email)
    if (response.error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    const response = await api.deleteUserInvitation(id)
    if (!response.error) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600">Invite new users to the platform</p>
      </div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          <li>1. Enter the email address of the person you want to invite</li>
          <li>2. They will receive an email with a registration link</li>
          <li>3. The link is valid for 7 days</li>
          <li>4. Once they complete registration, they can log in and use the platform</li>
        </ul>
      </div>
    </div>
  )
